...
def draw(image, boxes, scores, classes):
    ...
    # image = cv2.resize(image, (1200,720), interpolation = cv2.INTER_AREA)
...
def yolov5_post_process(input_data, conf_thre=0.3, nms_thre=0.45, model_size=416):
    ...
    for input, mask in zip(input_data, masks):
        b, c, s = process(input, mask, anchors, nms_thre, model_size)
        b, c, s = filter_boxes(b, c, s, conf_thre)
        boxes.append(b)
        classes.append(c)
        scores.append(s)
...
def process(input, mask, anchors, box_class_confidence, model_size):
    ...
    box_xy += grid
    box_xy *= int(model_size/grid_h)

    box_wh = pow(sigmoid(input[..., 2:4])*2, 2)
    box_wh = box_wh * anchors
    # box_wh = 4*pow(sigmoid(input[..., 2:4]), 2)
    # box_wh = box_wh * anchors
